#!/usr/bin/env python3
"""
ИСПРАВЛЕННЫЙ генератор презентации о VPN с PDF экспортом
Исправления:
1. Убраны номера слайдов из правого верхнего угла
2. PDF в альбомной ориентации как HTML
"""
...
log_filename = f"/home/ubuntu/openmanus_project/logs/vpn_fixed_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def create_vpn_presentation_fixed():
    """Создает ИСПРАВЛЕННУЮ презентацию о VPN"""
    
    logger.info("🚀 Создание ИСПРАВЛЕННОЙ презентации о VPN-сервере")
    start_time = time.time()
    
    # Та же презентация, что и раньше
    presentation = {
        ...
        "metadata": {
            "created_by": "OpenManus SlidesMode v2.0 + Qwen (ИСПРАВЛЕННАЯ ВЕРСИЯ)",
            ...
            "includes_code": True,
            "fixes": ["Убраны номера слайдов", "PDF в альбомной ориентации"]
        }
    }
    ...
    # Сохранение JSON
    json_path = "/home/ubuntu/openmanus_project/qwen_vpn_presentation_fixed.json"
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(presentation, f, indent=2, ensure_ascii=False)
    logger.info(f"📄 JSON сохранен: {json_path}")
    
    # Генерация ИСПРАВЛЕННОГО HTML
    html_content = generate_fixed_html(presentation)
    html_path = "/home/ubuntu/openmanus_project/qwen_vpn_presentation_fixed.html"
    with open(html_path, 'w', encoding='utf-8') as f:
        f.write(html_content)
    logger.info(f"🌐 HTML сохранен: {html_path}")
    
    # Генерация ИСПРАВЛЕННОГО PDF (альбомная ориентация)
    pdf_path = "/home/ubuntu/openmanus_project/qwen_vpn_presentation_fixed.pdf"
    try:
        subprocess.run([
            'wkhtmltopdf',
            '--page-size', 'A4',
        ...
        logger.info(f"📋 PDF создан в альбомной ориентации: {pdf_path}")
    ...
    logger.info("🎉 ИСПРАВЛЕННАЯ генерация завершена!")
    logger.info(f"⏱️ Время выполнения: {total_time:.2f} секунд")
    logger.info("🔧 Исправления:")
    logger.info("   ✅ Убраны номера слайдов из правого верхнего угла")
    logger.info("   ✅ PDF теперь в альбомной ориентации как HTML")
    ...
    return {
        ...
        "fixes_applied": [
            "Убраны номера слайдов",
            "PDF в альбомной ориентации"
        ]
    }

def generate_fixed_html(presentation):
    """Генерирует ИСПРАВЛЕННЫЙ HTML без номеров слайдов и с альбомной ориентацией"""
...
<body>
    <div class="title-page">
        ...
        <div style="margin-top: 30px; font-size: 0.9em; opacity: 0.7;">
            ...
            {datetime.now().strftime('%d.%m.%Y')} • ИСПРАВЛЕННАЯ ВЕРСИЯ
        </div>
    </div>
"""
    ...
    for i, slide in enumerate(slides, 1):
        ...
        # Используем сетку для лучшего размещения контента в альбомной ориентации
    ...
    html += f'''    <div class="metadata">
        ...
        <p><strong>Исправления:</strong> {', '.join(metadata.get('fixes', []))}</p>
    </div>
'''
...
def main():
    """Главная функция"""
    print("🔧 Создание ИСПРАВЛЕННОЙ VPN презентации...")
    print("✅ Исправления:")
    print("   • Убраны номера слайдов из правого верхнего угла")
    print("   • PDF теперь в альбомной ориентации как HTML")
    
    try:
        result = create_vpn_presentation_fixed()
        
        if result.get("success"):
            print("\n🎉 ИСПРАВЛЕННАЯ презентация создана!")
            ...
            print(f"🔧 Применены исправления: {', '.join(result['fixes_applied'])}")
